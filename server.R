shinyServer(function(input, output, session) {
    ...
    output$grafico_corredores_biologicos_especies_resumen <-
        renderPlotly({
    ...
    output$mapa_registros_presencia <- renderLeaflet({
        # areas_conservacion_filtradas <- filtrarAreasConservacion()
        registros_presencia_filtrados <- filtrarRegistrosPresencia()
        corredores_biologicos_filtrados <-
            filtrarCorredoresBiologicos()
    ...
    output$tabla_registros_presencia <- renderDT({
        registros_presencia_filtrados <- filtrarRegistrosPresencia()
        
        registros_presencia_filtrados %>%
            st_drop_geometry() %>%
            select(
                grupos_nomenclaturales,
                scientificName,
                locality,
                eventDate,
                corredor_biologico,
    ...
    output$grafico_corredores_biologicos_especies <-
        renderPlotly({
            corredores_biologicos_filtrados <- filtrarCorredoresBiologicos()
    ...
    output$grafico_estacionalidad_registros <-
        renderPlotly({
            registros_presencia_filtrados <- filtrarRegistrosPresencia()
    ...
    output$grafico_estacionalidad_especies <-
        renderPlotly({
            registros_presencia_filtrados <- filtrarRegistrosPresencia()
            ...
            registros_presencia_filtrados %>%
                st_drop_geometry() %>%
                group_by(mes = format(as.Date(eventDate, "%Y-%m-%d"), "%m")) %>%
                summarize(suma_especies = n_distinct(scientificName, na.rm = TRUE)) %>%
                filter(!is.na(mes))  %>%
                plot_ly(x = ~ mes,
                        y = ~ suma_especies,
                        type="bar", color = "green") %>%
